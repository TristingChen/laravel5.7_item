...
class ArrayHelp
{
    ...
    //获取当前路由
    public static function get_curent_action()
    {

        $routerStr = Route::current()->getActionName();
        list($class, $action) = explode('@', $routerStr);
        $controllerArr = explode('\\', $class);
        $controller = end($controllerArr);
        $controllerName = preg_replace('/^([\S]*?)Controller$/', '$1', $controller);

        return [
            'class' => $class,
            'controller' => $controller,
            'controller_name' => strtolower($controllerName),
            'action' => $action
        ];
    }
